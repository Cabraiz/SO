...
public class RoundRobin extends Builder {
                    ...
                    for (int i = 0; exeLista.size() > i && aptoLista.size() > i; i++) {
                        ...
                        for (int j = 0; j < exeLista.size(); j++) {
                            ...
                            if (exeLista.get(j).ID == 0) {
                                Processo n = new Processo(p.getID(), p.getExe(),
                                        p.getDeadline(), p.getPrior(), nowTime, nowTime, p.getMemory(), p.isWorking(), p.oldMemory);
                        ...
                        if (p.createTime != 0) {
                            ...
                            if (nowTime > p.createTime + SaveLogic.quantum * 1000 || p.exe < 0) {
                                if (p.exe < 0) {
                                    ...
                                    aptoLista.add(new Processo(p.getID(), p.exe - runTime,
                                            p.getDeadline(), p.getPrior(), p.getCreateTime(),
                                            p.getMemory(), p.isWorking(), p.oldMemory));
                                    exeLista.set(exeLista.indexOf(p), new Processo());
                                }
                            } else {
                                if (exeLista.indexOf(p) != -1) {
                                    exeLista.set(exeLista.indexOf(p),
                                            new Processo(p.getID(), p.getExe() - runTime,
                                                    p.getDeadline(), p.getPrior(), p.getCreateTime(), nowTime,
                                                    p.getMemory(), p.isWorking(), p.oldMemory));
                                }
                            }
                        }
    ...
    private void initComponents() {
        ...
        setPreferredSize(new java.awt.Dimension(633, 554));
